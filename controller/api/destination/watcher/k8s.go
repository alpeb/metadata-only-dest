...
func InitializeIndexers(k8sAPI *k8s.API, k8sMetadataAPI *k8s.MetadataAPI) error {
	ix := cache.Indexers{PodIPIndex: func(obj interface{}) ([]string, error) {
	...
	err := k8sAPI.Svc().Informer().AddIndexers(ix)
	if err != nil {
		return fmt.Errorf("could not create an indexer for services: %w", err)
	}

	inf, err := k8sMetadataAPI.GetInformer(k8s.Svc)
	if err != nil {
		return err
	}
	err = inf.AddIndexers(ix)
	if err != nil {
		return fmt.Errorf("could not create an indexer for services: %w (metadata API)", err)
	}
	...
	err = k8sAPI.Pod().Informer().AddIndexers(ix)
	if err != nil {
		return fmt.Errorf("could not create an indexer for pods: %w", err)
	}

	inf, err = k8sMetadataAPI.GetInformer(k8s.Pod)
	if err != nil {
		return err
	}
	err = inf.AddIndexers(ix)
	if err != nil {
		return fmt.Errorf("could not create an indexer for pods: %w (metadata API)", err)
	}

	ix = cache.Indexers{HostIPIndex: func(obj interface{}) ([]string, error) {
		if pod, ok := obj.(*corev1.Pod); ok {
